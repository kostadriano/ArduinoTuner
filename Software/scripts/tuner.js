...
var Tuner = function (opts) {
	...
	document.getElementById("currentString").innerHTML = currentString;
	
	document.getElementById("stringE").onclick=function(){currentStringChange("E")};
	document.getElementById("stringA").onclick=function(){currentStringChange("A")};
	document.getElementById("stringD").onclick=function(){currentStringChange("D")};
	document.getElementById("stringG").onclick=function(){currentStringChange("G")};
	document.getElementById("stringB").onclick=function(){currentStringChange("B")};
	document.getElementById("stringe").onclick=function(){currentStringChange("e")};
...
Tuner.Source.Audio.prototype.init = function (tuner) {
	...
	try {
		...
		var usermedia = navigator.getUserMedia({
			audio: audio_options
		}, this.connect_success.bind(this), this.connect_error.bind(this));
	} catch (e) {
...
var postData = "Some data";
	...
	var intervalID = window.setInterval(function() {
		request.open("POST", "http://192.168.0.100/", true);
		var sendFrequency = parseFloat(tuner.frequency().toFixed(3));
		request.send("$"+sendFrequency+"$"+"*"+currentString+"*");
		}, 1000);
		this.tuner.tick();
	} catch (e) {
		console.debug(e);
...
function currentStringChange(newString){
